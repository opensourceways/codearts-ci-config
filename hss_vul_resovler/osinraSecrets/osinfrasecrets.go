...
func main() {
	url := "https://secret.osinfra.cn/v1/secrets/metadata/"
...
func processSecrets(groupPath string) {
	...
	handler.DeleteRecordsByWhere("jenkins_vault_secrets", fmt.Sprintf("1=1 and 项目='%s'", groupPath))
	...
	handler.InsertJsonRecords2SqlLiteDb("jenkins_vault_secrets", records, "项目", "服务", "key")
}

func getSecretCurrentVersion(path string) map[string]interface{} {
	url := "https://secret.osinfra.cn/v1/secrets/data/" + path
	method := "GET"
