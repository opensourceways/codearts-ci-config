...
org=$(basename $(dirname "$remote_url"))
...
pwd
git clone https://oauth2:${access_token}@gitcode.com/${org}/$repo.git $repo
...
python3 check_gitleaks.py /opt/cached_resources/confirmed/repo_gitleaks_confirmed_fingerprints_startColumn.json  /opt/cached_resources/confirmed/repo_gitleaks_withGitRepoHint_fingerprints_startColumn.json /opt/cached_resources/gitleaks/${cleaned__repo_url}/result-${sanitizedBranch}-${TIMESTAMP}.json ${org}/${repo}
